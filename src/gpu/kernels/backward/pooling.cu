...
// Optimized MaxPool2D backward (scatter-only, zero-init done in host wrapper)
__global__ void maxpool2dBackwardOptKernel(
    ...
    if (outIdx >= totalOutputs) return;
    
    int c = outIdx % channels;
    int w = (outIdx / channels) % outW;
    int h = (outIdx / (channels * outW)) % outH;
    int n = outIdx / (channels * outW * outH);
    ...
    int in_h = h * 2 + kh;
    int in_w = w * 2 + kw;
    int gradInputIdx = ((n * inH + in_h) * inW + in_w) * channels + c;
    
    atomicAdd(&gradInput[gradInputIdx], gradOutput[outIdx]);
}
...
// Optimized Upsample2D backward with unrolled loop
__global__ void upsample2dBackwardOptKernel(
    const float* __restrict__ gradOutput,
    float* __restrict__ gradInput,
    int batch, int inH, int inW, int channels
) {
    ...
    for (int kh = 0; kh < 2; kh++) {
        #pragma unroll
        for (int kw = 0; kw < 2; kw++) {
            int out_h = base_out_h + kh;
            int out_w = base_out_w + kw;
            int outputIdx = ((n * outH + out_h) * outW + out_w) * channels + c;
            sum += gradOutput[outputIdx];
        }
    }
...
// Fused Upsample + ReLU backward (GPU Opt v2)
__global__ void fusedUpsampleReluBackwardKernel(
    const float* __restrict__ gradOutput,
    const float* __restrict__ reluInput,
    float* __restrict__ gradInput,
    int batch, int inH, int inW, int channels
) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
void launchMaxPool2dBackward(
    ...
    int gridSizeInput = (inputThreads + blockSize - 1) / blockSize;
    maxpool2dBackwardZeroKernel<<<gridSizeInput, blockSize>>>(d_gradInput, batch, inH, inW, channels);
    
    // Scatter gradients
    int outH = inH / 2;
    int outW = inW / 2;
    int outputThreads = batch * outH * outW * channels;
    int gridSizeOutput = (outputThreads + blockSize - 1) / blockSize;
    maxpool2dBackwardScatterKernel<<<gridSizeOutput, blockSize>>>(
        d_gradOutput, d_indices, d_gradInput, batch, inH, inW, channels
    );
    
    CHECK_CUDA(cudaGetLastError());
}

void launchMaxPool2dBackwardOpt(
    const float* d_gradOutput, const int* d_indices, float* d_gradInput,
    int batch, int inH, int inW, int channels
) {
    // Zero-initialize gradInput (safe, no race condition)
    size_t inputSize = static_cast<size_t>(batch) * inH * inW * channels;
    ...
    maxpool2dBackwardOptKernel<<<gridSize, blockSize>>>(
        d_gradOutput, d_indices, d_gradInput, batch, inH, inW, channels
    );
    CHECK_CUDA(cudaGetLastError());
}
...
void launchUpsample2dBackward(
    ...
    int totalInputs = batch * inH * inW * channels;
    int blockSize = CUDA_BLOCK_SIZE;
    int gridSize = (totalInputs + blockSize - 1) / blockSize;
    
    upsample2dBackwardKernel<<<gridSize, blockSize>>>(
        d_gradOutput, d_gradInput, batch, inH, inW, channels
    );
    CHECK_CUDA(cudaGetLastError());
}

void launchUpsample2dBackwardOpt(
    const float* d_gradOutput, float* d_gradInput,
    int batch, int inH, int inW, int channels
) {
    int totalInputs = batch * inH * inW * channels;
    ...
    upsample2dBackwardOptKernel<<<gridSize, blockSize>>>(
        d_gradOutput, d_gradInput, batch, inH, inW, channels
    );
    CHECK_CUDA(cudaGetLastError());
}

void launchFusedUpsampleReluBackward(
    const float* d_gradOutput, const float* d_reluInput, float* d_gradInput,
    int batch, int inH, int inW, int channels
) {
    int totalInputs = batch * inH * inW * channels;
    int blockSize = CUDA_BLOCK_SIZE;
    ...
    fusedUpsampleReluBackwardKernel<<<gridSize, blockSize>>>(
        d_gradOutput, d_reluInput, d_gradInput, batch, inH, inW, channels
    );
    CHECK_CUDA(cudaGetLastError());
}
...
// Stream-aware NCHW MaxPool2D backward
void launchMaxPool2dBackwardNCHWStream(
    const float* d_gradOutput, const int* d_indices, float* d_gradInput,
    int batch, int channels, int inH, int inW,
    int k, int stride,
    cudaStream_t stream
    ...
    // Zero-initialize gradInput on stream
    size_t inputSize = static_cast<size_t>(batch) * channels * inH * inW;
    CHECK_CUDA(cudaMemsetAsync(d_gradInput, 0, inputSize * sizeof(float), stream));
    ...
    maxpool2dBackwardNCHWKernel<<<grid, block, 0, stream>>>(
        d_gradOutput, d_indices, d_gradInput,
        batch, channels, inH, inW, outH, outW, k, stride
    );
    CHECK_CUDA(cudaGetLastError());
...
// Stream-aware NCHW Upsample2D backward
void launchUpsample2dBackwardNCHWStream(
    const float* d_gradOutput, float* d_gradInput,
    int batch, int channels, int inH, int inW,
    int scale,
    cudaStream_t stream
    ...
    upsample2dBackwardNCHWKernel<<<grid, block, 0, stream>>>(
        d_gradOutput, d_gradInput,
        batch, channels, inH, inW, outH, outW, scale
    );
    CHECK_CUDA(cudaGetLastError());
