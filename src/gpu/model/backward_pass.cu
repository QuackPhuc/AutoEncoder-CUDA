...
// ============================================================
// GPU Opt V3: im2col + cuBLAS GEMM + CUDA Streams Backward Pass
// ============================================================
...
extern void launchCol2imNCHWStream(
    const float* d_col, float* d_gradInput,
    int batch, int inC, int inH, int inW,
    int outH, int outW,
    int kernelSize, int stride, int padding,
...
extern void launchConvGemmBackwardInputStream(
    const float* d_weights, const float* d_gradOutput, float* d_col,
    int batch, int outC, int inC_k_k, int outHW,
    cublasHandle_t handle, cudaStream_t stream);

extern void launchConvGemmBackwardWeightsStream(
    const float* d_gradOutput, const float* d_im2col, float* d_gradWeights,
    int batch, int outC, int inC_k_k, int outHW,
    cublasHandle_t handle, cudaStream_t stream);

extern void launchMaxPool2dBackwardNCHWStream(
    const float* d_gradOutput, const int* d_indices, float* d_gradInput,
    int batch, int channels, int inH, int inW,
    int k, int stride, cudaStream_t stream);

extern void launchUpsample2dBackwardNCHWStream(
    const float* d_gradOutput, float* d_gradInput,
    int batch, int channels, int inH, int inW,
    int scale, cudaStream_t stream);

void GPUAutoencoder::backwardOptV3() {
    ...
    // MSE gradient on default stream first
    int gridSize = (outputSize + blockSize - 1) / blockSize;
    mseLossGradKernel<<<gridSize, blockSize>>>(d_output, d_target, d_grad_output, outputSize);
    CHECK_CUDA(cudaDeviceSynchronize()); // Ensure gradient is ready
    
    // ========================
    // Layer 5 backward: Conv only (no ReLU)
    // ========================
    // Backward input: dX = col2im(W^T × dY)
    launchConvGemmBackwardInputStream(d_dec_conv5_w, d_grad_output, d_im2col_workspace,
                                      m_batchSize, 3, 2304, 1024, m_cublas_handle, s_comp);
    CHECK_CUDA(cudaStreamSynchronize(s_comp));
    
    launchCol2imNCHWStream(d_im2col_workspace, d_grad_dec_up2,
                           m_batchSize, 256, 32, 32, 32, 32, 3, 1, 1, s_aux);
    
    // Backward weights (can run in parallel with col2im)
    launchIm2colNCHWStream(d_dec_up2_out, d_im2col_workspace,
                           m_batchSize, 256, 32, 32, 32, 32, 3, 1, 1, s_aux);
    CHECK_CUDA(cudaStreamSynchronize(s_aux));
    
    launchConvGemmBackwardWeightsStream(d_grad_output, d_im2col_workspace, d_dec_conv5_grad_w,
                                        m_batchSize, 3, 2304, 1024, m_cublas_handle, s_comp);
    launchConv2dBackwardBiasNCHW(d_grad_output, d_dec_conv5_grad_b, m_batchSize, 3, 32, 32);
    CHECK_CUDA(cudaStreamSynchronize(s_comp));
    ...
    // Layer 4 backward: Upsample + ReLU + Conv
    // ========================
    launchUpsample2dBackwardNCHWStream(d_grad_dec_up2, d_grad_dec_relu4, 
                                       m_batchSize, 256, 16, 16, 2, s_aux);
    CHECK_CUDA(cudaStreamSynchronize(s_aux));
    
    int relu4Size = m_batchSize * 256 * 16 * 16;
    reluBackwardKernel<<<(relu4Size + 255) / 256, 256, 0, s_aux>>>(
        d_grad_dec_relu4, d_dec_relu4_out, d_grad_dec_conv4, relu4Size);
    CHECK_CUDA(cudaStreamSynchronize(s_aux));
    
    launchConvGemmBackwardInputStream(d_dec_conv4_w, d_grad_dec_conv4, d_im2col_workspace,
                                      m_batchSize, 256, 1152, 256, m_cublas_handle, s_comp);
    CHECK_CUDA(cudaStreamSynchronize(s_comp));
    
    launchCol2imNCHWStream(d_im2col_workspace, d_grad_dec_up1,
                           m_batchSize, 128, 16, 16, 16, 16, 3, 1, 1, s_aux);
    launchIm2colNCHWStream(d_dec_up1_out, d_im2col_workspace,
                           m_batchSize, 128, 16, 16, 16, 16, 3, 1, 1, s_aux);
    CHECK_CUDA(cudaStreamSynchronize(s_aux));
    
    launchConvGemmBackwardWeightsStream(d_grad_dec_conv4, d_im2col_workspace, d_dec_conv4_grad_w,
                                        m_batchSize, 256, 1152, 256, m_cublas_handle, s_comp);
    launchConv2dBackwardBiasNCHW(d_grad_dec_conv4, d_dec_conv4_grad_b, m_batchSize, 256, 16, 16);
    CHECK_CUDA(cudaStreamSynchronize(s_comp));
    ...
    // Layer 3 backward: Upsample + ReLU + Conv
    // ========================
    launchUpsample2dBackwardNCHWStream(d_grad_dec_up1, d_grad_dec_relu3,
                                       m_batchSize, 128, 8, 8, 2, s_aux);
    CHECK_CUDA(cudaStreamSynchronize(s_aux));
    
    int relu3Size = m_batchSize * 128 * 8 * 8;
    reluBackwardKernel<<<(relu3Size + 255) / 256, 256, 0, s_aux>>>(
        d_grad_dec_relu3, d_dec_relu3_out, d_grad_dec_conv3, relu3Size);
    CHECK_CUDA(cudaStreamSynchronize(s_aux));
    
    launchConvGemmBackwardInputStream(d_dec_conv3_w, d_grad_dec_conv3, d_im2col_workspace,
                                      m_batchSize, 128, 1152, 64, m_cublas_handle, s_comp);
    CHECK_CUDA(cudaStreamSynchronize(s_comp));
    
    launchCol2imNCHWStream(d_im2col_workspace, d_grad_enc_pool2,
                           m_batchSize, 128, 8, 8, 8, 8, 3, 1, 1, s_aux);
    launchIm2colNCHWStream(d_enc_pool2_out, d_im2col_workspace,
                           m_batchSize, 128, 8, 8, 8, 8, 3, 1, 1, s_aux);
    CHECK_CUDA(cudaStreamSynchronize(s_aux));
    
    launchConvGemmBackwardWeightsStream(d_grad_dec_conv3, d_im2col_workspace, d_dec_conv3_grad_w,
                                        m_batchSize, 128, 1152, 64, m_cublas_handle, s_comp);
    launchConv2dBackwardBiasNCHW(d_grad_dec_conv3, d_dec_conv3_grad_b, m_batchSize, 128, 8, 8);
    CHECK_CUDA(cudaStreamSynchronize(s_comp));
    ...
    // Layer 2 backward: MaxPool + ReLU + Conv
    // ========================
    launchMaxPool2dBackwardNCHWStream(d_grad_enc_pool2, d_enc_pool2_indices, d_grad_enc_relu2,
                                      m_batchSize, 128, 16, 16, 2, 2, s_aux);
    CHECK_CUDA(cudaStreamSynchronize(s_aux));
    
    int relu2Size = m_batchSize * 128 * 16 * 16;
    reluBackwardKernel<<<(relu2Size + 255) / 256, 256, 0, s_aux>>>(
        d_grad_enc_relu2, d_enc_relu2_out, d_grad_enc_conv2, relu2Size);
    CHECK_CUDA(cudaStreamSynchronize(s_aux));
    
    launchConvGemmBackwardInputStream(d_enc_conv2_w, d_grad_enc_conv2, d_im2col_workspace,
                                      m_batchSize, 128, 2304, 256, m_cublas_handle, s_comp);
    CHECK_CUDA(cudaStreamSynchronize(s_comp));
    
    launchCol2imNCHWStream(d_im2col_workspace, d_grad_enc_pool1,
                           m_batchSize, 256, 16, 16, 16, 16, 3, 1, 1, s_aux);
    launchIm2colNCHWStream(d_enc_pool1_out, d_im2col_workspace,
                           m_batchSize, 256, 16, 16, 16, 16, 3, 1, 1, s_aux);
    CHECK_CUDA(cudaStreamSynchronize(s_aux));
    
    launchConvGemmBackwardWeightsStream(d_grad_enc_conv2, d_im2col_workspace, d_enc_conv2_grad_w,
                                        m_batchSize, 128, 2304, 256, m_cublas_handle, s_comp);
    launchConv2dBackwardBiasNCHW(d_grad_enc_conv2, d_enc_conv2_grad_b, m_batchSize, 128, 16, 16);
    CHECK_CUDA(cudaStreamSynchronize(s_comp));
    ...
    // Layer 1 backward: MaxPool + ReLU + Conv (no input gradient needed)
    // ========================
    launchMaxPool2dBackwardNCHWStream(d_grad_enc_pool1, d_enc_pool1_indices, d_grad_enc_relu1,
                                      m_batchSize, 256, 32, 32, 2, 2, s_aux);
    CHECK_CUDA(cudaStreamSynchronize(s_aux));
    
    int relu1Size = m_batchSize * 256 * 32 * 32;
    reluBackwardKernel<<<(relu1Size + 255) / 256, 256, 0, s_aux>>>(
        d_grad_enc_relu1, d_enc_relu1_out, d_grad_enc_conv1, relu1Size);
    CHECK_CUDA(cudaStreamSynchronize(s_aux));
    
    // Only backward weights for first layer
    launchIm2colNCHWStream(d_input, d_im2col_workspace,
                           m_batchSize, 3, 32, 32, 32, 32, 3, 1, 1, s_aux);
    CHECK_CUDA(cudaStreamSynchronize(s_aux));
    
    launchConvGemmBackwardWeightsStream(d_grad_enc_conv1, d_im2col_workspace, d_enc_conv1_grad_w,
                                        m_batchSize, 256, 27, 1024, m_cublas_handle, s_comp);
    launchConv2dBackwardBiasNCHW(d_grad_enc_conv1, d_enc_conv1_grad_b, m_batchSize, 256, 32, 32);
