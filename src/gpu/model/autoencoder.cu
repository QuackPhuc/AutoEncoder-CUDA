...
std::vector<float> GPUAutoencoder::extractBatchFeatures(const std::vector<float>& images, int numImages) {
    // Delegate to pointer-based implementation (avoids copy when caller uses this interface)
    return extractBatchFeatures(images.data(), numImages);
}

std::vector<float> GPUAutoencoder::extractBatchFeatures(const float* images, int numImages) {
    ...
    CHECK_CUDA(cudaMemcpy(d_input, images, inputBytes, cudaMemcpyHostToDevice));
